...
class EmailToBot:
    ...
    async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
        await context.bot.send_message(chat_id=update.effective_chat.id, text="Hello, I can help you to check email!")
    ...
    async def join(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if text_caps == my_key:
            if update.effective_chat.id not in all_chat_id:
                ...
                await context.bot.send_message(chat_id=update.effective_chat.id, text='Congratulations, you successfully join the session!')
            else:
                await context.bot.send_message(chat_id=update.effective_chat.id, text='Oh, you already join the session!')
        else:
            await context.bot.send_message(chat_id=update.effective_chat.id, text='Sorry, the password is incorrect, please try again.')

    async def leave(update: Update, context: ContextTypes.DEFAULT_TYPE):
        if update.effective_chat.id not in all_chat_id:
            await context.bot.send_message(chat_id=update.effective_chat.id, text='You have not joined the session.')
        else:
            all_chat_id.remove(update.effective_chat.id)
            await context.bot.send_message(chat_id=update.effective_chat.id, text='Byebye, you successfully leave the session!')
